...
#the Rule function requires 2 inputs atleast so we defined this simple input that is always true
simple = Domain("dis", 0, 1)
simple.test = trapezoid(-5, -2, 1, 2)

#setting our input domain (range of values) and membership functions
Ball = Domain("Distance", 0, 3, res=0.1)
Ball.low = trapezoid(-1, -0.5, 0.5, 1.5)
Ball.mid = triangular(0.5, 2.5)
Ball.high = trapezoid(1.5, 2.5, 3, 4)

#setting the output domain and membership functions
V = Domain("Voltage", 9, 12, res=0.1)
V.low = trapezoid(4, 5, 9.5, 10.5)
V.mid = triangular(9.5, 11.5)
V.high = trapezoid(10.5, 11.5, 13, 14)

#setting our rules
R1 = Rule({(Ball.high, simple.test): V.high})
R2 = Rule({(Ball.mid, simple.test): V.mid})
R3 = Rule({(Ball.low, simple.test): V.low})
rules = Rule({(Ball.high, simple.test): V.high,
             (Ball.mid, simple.test): V.mid,
             (Ball.low, simple.test): V.low})
rules == R1 | R2 | R3 == sum([R1, R2, R3])
...
for i in range(1000): # check the distance 100 times
    dist = round(check_distance(), 1)
    values = {Ball: (dist - 4), simple: 1}
    Voltage = rules(values)
